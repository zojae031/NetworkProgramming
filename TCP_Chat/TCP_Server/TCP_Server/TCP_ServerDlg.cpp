...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, panel);
	DDX_Control(pDX, IDC_BUTTON1, b1);
	DDX_Control(pDX, IDC_BUTTON2, b2);
	DDX_Control(pDX, IDC_BUTTON3, all);
...
BEGIN_MESSAGE_MAP(CTCPServerDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON3, &CTCPServerDlg::OnBnClickedButton3)
	ON_BN_CLICKED(IDOK, &CTCPServerDlg::OnBnClickedOk)
END_MESSAGE_MAP()
...
void CTCPServerDlg::OnBnClickedButton1()
{
	...
	CString data;
	for (int i = 0; i < server1->nTotalSockets; i++) {
		CString id = server1->SocketInfoArray[i]->id;
		data.AppendFormat("%s\r\n", id);
	}
	
	panel.SetWindowText(data);
}
...
void CTCPServerDlg::OnBnClickedButton2()
{
	//2번방 버튼
	CString data;
	for (int i = 0; i < server2->nTotalSockets; i++) {
		CString id = server2->SocketInfoArray[i]->id;
		data.AppendFormat("%s\r\n", id);
	}
...
void CTCPServerDlg::OnBnClickedOk()
{
	...
	server1->openServer();
	server2->openServer();
	status.SetWindowText("Server Open!!");
	b1.EnableWindow(true);
	b2.EnableWindow(true);
	all.EnableWindow(true);
